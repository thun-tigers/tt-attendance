...
from app.authz import normalize_auth_payload
...
def test_normalize_auth_payload_preserves_coach_roles():
    payload = normalize_auth_payload({
        'role': 'coach',
        'platform_role': 'head_coach',
        'memberships': [{'team_code': 'U18'}],
    })

    assert payload['service_role'] == 'coach'
    assert payload['platform_role'] == 'head_coach'


def test_normalize_auth_payload_preserves_team_betreuer_role():
    payload = normalize_auth_payload({'role': 'team_betreuer'})

    assert payload['service_role'] == 'team_betreuer'
