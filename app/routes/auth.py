...
def _safe_next_endpoint(next_page):
    if not next_page:
        return None, None
    ...
    adapter = current_app.url_map.bind(ref_url.netloc, url_scheme=ref_url.scheme)
    try:
        endpoint, values = adapter.match(test_url.path or '/', method='GET')
...
def sso_login():
    ...
    next_endpoint, next_values = _safe_next_endpoint(request.args.get('next'))
    if next_endpoint:
        return redirect(url_for(next_endpoint, **next_values))
